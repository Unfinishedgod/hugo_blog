---
title: '[Airflow] 주가 데이터 수집 파이프라인 4 (환율 편)'
author: 최의용
date: '2023-08-09'
slug: airflow-4
categories:
...
이번에는 날짜에 대한 세팅을 해주도록 하자. 환율 정보는 월~금요일 오전 9시에 수집이 될 예정이다. 그리고 start_date2를 today_date2와 동일하게 해주었다. 이는 `fdr.DataReader()` 함수의 경우 시작날짜와 종료날짜가 모두 있어야 하기 때문이다.
...
환율 데이터를 수집 해주자. fdr의 fdr.DataReader()함수를 사용 했다. 기존 S&P 500 데이터를 수집할 때 `fdr.DataReader()`에서 ticker를 넣어 주었지만 이번에는 'USD/KRW' 옵션을 넣어 주면 된다.
...
- [finance_mlops/python_file/usd_krw_crawler.py](https://github.com/Unfinishedgod/finance_mlops/blob/main/python_file/usd_krw_crawler.py)
