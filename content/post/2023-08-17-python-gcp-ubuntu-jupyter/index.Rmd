...
GCP에서 구축한 Compute Engine에 콘다를 설치 해보자. 추가로 이번에 Jupyter notebook에 데몬 프로세스를 추가 해서 서버가 켜질때마다 자동으로 주피터도 같이 실행을 하는 방법을 알아보려고 한다. 이 블로그는 다음의 선 작업이 진행된 후에 작업을 하는 것이 적합하다.
