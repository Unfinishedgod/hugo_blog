---
title: '[Python] 파인튜닝을 통해 나만의 데이터로 학습을 시켜보자'
author: 최의용
date: '2024-05-24'
slug: python
categories:
...
- 학습을 위한 데이터: [Huggingface 데이터셋 업로드 참고](https://unfinishedgod.netlify.app/2024/05/19/python-huggingface/)

## 참고

2023년 Llama2 이후로 상당히 파인튜닝에 대해 많은 글이 쏟아지고 있다. 그러다 2024년에 Llama3가 나오게 되면서 파인튜닝에 대해 여러 영상과 글을 읽어 봤는데 다음의 유튜브가 많은 참고가 되었다. 

- [HK CODE님의 Llama3 Model Finetune](https://www.youtube.com/watch?v=f8pDjOkXIZo)
...
- datasets: Hugging Face에서 제공하는 라이브러리로, 데이터셋을 쉽게 로드할 수 있다. (참고: [huggingface 데이터셋 업로드](https://unfinishedgod.netlify.app/2024/05/19/python-huggingface/))
...
- [Huggingface에 데이터셋 업로드](https://unfinishedgod.netlify.app/2024/05/19/python-huggingface/)
...
- [teddylee님의 HuggingFace](https://huggingface.co/teddylee777)
- [beomi님의 HuggingFace](https://huggingface.co/beomi)
...
#### 고성능 GPU
- `torch.cuda.get_device_capability()[0] >= 8` 조건문을 통해 현재 사용중인 GPU의 CUDA 연산 능력을 확인한다. 8이상이면 고성능 GPU 로 판단한다.
- Attention 메커니즘 선택: 고성능 Attention인 flash attention 2 을 사용해준다.
- 데이터 타입을 bfloat16으로 설정해준다. bfloat16은 메모리 사용량을 줄이면서도 계산의 정확성을 유지할 수 있는 데이터 타입이다.
  - [Google문서: bfloat16 숫자 형식](https://cloud.google.com/tpu/docs/bfloat16?hl=ko)
...
- bnb_4bit_compute_dtype=torch_dtype: 양자화를 위한 컴퓨팅 타입은 직전에 정의 했던 `torch_dtype`으로 지정 해준다.
...
- AutoModelForCausalLM.from_pretrained()
  ...
  - device_map={"": 0}: 모델을 특정 장치에 할장한다. 여기서 `{"": 0}`는 모델을 첫 번재 GPU에 할당 하겠다는 의미.
...
실행하면 다음과 같이 모델들을 불러오게 된다. 시간은 약 3분정도 소요 된다.
...
<center>
![](images/파인튜닝 결과 메모리.JPG){width=80%}
</center>
...
- [uiyong/kospi_report_model_0517](https://huggingface.co/uiyong/kospi_report_model_0517)

```python
model.push_to_hub("uiyong/kospi_report_model_0517")
tokenizer.push_to_hub("uiyong/kospi_report_model_0517")
```
